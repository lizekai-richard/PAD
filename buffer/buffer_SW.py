...
def train(pid, args, channel, num_classes, im_size, trainloader, images_all, labels_all,
    ...
    for it in range(10):
        ...
        for e in range(args.train_epochs):
            ...
            if e < args.sw_start_epoch:
                p_add = linear_cl_scheduler_acse(e, args.init_ratio, args.add_end_epoch)
                p_rm = 0.0
                indices = sorted_diff_indices[:, :int(p_add * data_size_by_class)].flatten()
            elif args.sw_start_epoch < e <= args.add_end_epoch:
                p_add = linear_cl_scheduler_acse(e, args.init_ratio, args.add_end_epoch)
                p_rm = (e - args.sw_start_epoch) * args.rm_rate
                indices = sorted_diff_indices[:, int(p_rm * data_size_by_class):
                                                 int(p_add * data_size_by_class)].flatten()
            elif args.add_end_epoch < e <= args.rm_resume_epoch:
                p_add = 1.0
                p_rm = (args.rm_resume_epoch - args.sw_start_epoch) * args.rm_rate
                indices = sorted_diff_indices[:, int(p_rm * data_size_by_class):].flatten()
            else:
                p_add = 1.0
                p_rm = (e - args.sw_start_epoch) * args.rm_rate
                indices = sorted_diff_indices[:, int(p_rm * data_size_by_class):].flatten()
            ...
            print("Itr: {}\tEpoch: {}\tRatio: {}\tTrain Acc: {}\tTest Acc: {}".format(it, e, p_add - p_rm, train_acc,
                                                                                      test_acc))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--rm_rate', type=float, default=0.005)
    parser.add_argument('--rm_resume_epoch', type=int, default=60)
    parser.add_argument('--sw_start_epoch', type=int, default=20)
    parser.add_argument('--add_end_epoch', type=int, default=40)
